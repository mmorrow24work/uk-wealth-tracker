...
## A high "Clones" count on a public repo is (almost certainly) your own CI, not a leak

The repo's Traffic insights page showed 841 clones / 55 unique cloners over 14 days, which read as alarming on its own — enough to prompt a full check for accidentally-committed secrets or personal data. Worth writing down both the reassurance and the check, since an autonomous pipeline that pushes/merges/deploys this often will always produce a number that looks big in isolation.

**Why the number is high: it's not people.** GitHub's "Clones" metric counts every `git clone`/`git fetch` against the repo, and — critically — every `actions/checkout` step in every workflow run counts as one. This repo runs three workflows (`claude-build.yml`, `ci.yml`, `deploy.yml`); checking `gh api repos/<owner>/<repo>/actions/workflows/<id>/runs --jq '.total_count'` for each showed **533 total runs combined** at the time this was checked, each doing its own checkout. That alone explains the large majority of the 14-day count, before counting routine bot/scraper indexing that hits every public GitHub repo regardless of content, or a maintainer's own local `git fetch`/`git clone` traffic during active debugging. A repo with a very active CI pipeline will *always* show clone numbers that look disproportionate to real human interest — the metric doesn't distinguish an `actions/checkout` from a person.

**How to actually check for a leak, rather than just reassure yourself about the metric** (real commands, not just "should be fine"):
```sh
# Whole history, not just current files -- a since-deleted secret is still in every commit that
# ever had it, unless history was rewritten (git filter-repo / BFG), which is a separate, more
# drastic step only worth taking if this search finds something real.
git log --all -p | grep -oE "gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|sk-ant-[A-Za-z0-9-]{20,}|AKIA[0-9A-Z]{16}|-----BEGIN [A-Z ]*PRIVATE KEY-----"

# .env files that were ever tracked, even if later removed/gitignored
git log --all --diff-filter=A --name-only --format="" | grep -i "\.env"

# App-specific PII patterns worth searching for -- adjust to what the app actually stores.
# Here: UK National Insurance number shape, since this is a UK finance app.
git log --all -p | grep -oE "\b[A-CEGHJ-PR-TW-Z]{2}[0-9]{6}[A-D]\b"
```
Confirm secrets are genuinely encrypted, not just gitignored: `gh secret list` shows names/rotation dates only, never values, for anything stored as an Actions secret — if a token shows up there rather than in `gh variable list` (which *is* plaintext-visible) or in tracked files, it was never exposed via the repo itself.

**The app's own architecture is the real reason this is low-risk here, not just clean history:** this app has no backend by design (`DESIGN.md`) — real user data lives in the browser or the user's own private Gist, never in this codebase. The only way personal data could end up in the repo at all is a stray real figure pasted into a commit message, issue, or test fixture by hand; every issue/PR here was authored by the automated pipeline using synthetic example data throughout (test property addresses like "12 Oak Avenue", round example pension pots), which is a structurally different — and much lower — risk than an app that actually stores user data server-side.

**Standing takeaway for any public repo with an active bot/CI pipeline:** expect Traffic → Clones to look large and don't treat the raw number as a signal by itself. Cross-check it against total workflow run count first; only chase an actual content audit (the commands above) if that alone doesn't explain the gap, or if there's a concrete reason to suspect a specific commit.

---

## Real personal data reached a public repo, and what deleting it doesn't undo

The clones audit above was a false alarm — the repo checked out clean. A short time later, real personal data actually did land in this same repo: two files manually uploaded to `docs/` for reference (a genuine WealthR export with real pension policy numbers, and this app's own JSON export), on a `main` branch that was public at the time. See `docs/journal.md`'s own entry for the fuller incident writeup; this section is the reusable lesson.

**`git rm` + commit only affects the current tree, not history.** The two files are still fully recoverable from the upload commits themselves for as long as the repo exists — a normal deletion commit is not a mitigation for data that shouldn't have been public, it's just housekeeping. Anyone (a person, a bot, GitHub's own indexing) who cloned or fetched the repo during the window the files were live already has an independent copy that no amount of cleanup on the canonical repo can reach.

**A history rewrite (`git filter-repo` + force-push) *can* remove the files from a repo's own history, but has real teeth worth knowing before reaching for it:**
- Every commit from the affected one onward gets a new SHA — breaks existing PR links, and (in this repo specifically) any already-deployed footer's commit link.
- Requires force-pushing `main`. On a repo with an **actively-running autonomous pipeline that merges every 15–30 minutes**, doing this while a PR is open or a run is mid-flight risks the rewrite and the pipeline's own next push fighting each other — the pipeline could end up working from stale local state and effectively re-introducing what was just rewritten away.
- Doesn't reach anything that already left the repo before the rewrite (GitHub's own caching of "unreachable" objects after a force-push can lag its own garbage collection; any external clone/fork is untouched entirely).

**The lower-risk alternative, chosen here: don't rewrite history in place — relaunch clean instead.** Take the repo private immediately (stops all *further* public access, fully reversible, zero risk to the live pipeline — `claude-code-action`/`GH_PAT` work identically on a private repo). Let the pipeline finish its current backlog undisturbed. Then rename the repo to a `-private-archive-<date>` name (its full history stays intact, forever, just never public again) and stand up a brand new public repo from a single fresh commit of the finished code, with no history to carry over — nothing left to scrub because nothing from before is in it. Slower than a rewrite, but nothing to get subtly wrong, no risk to in-flight pipeline work, and every already-existing clone/fork problem is moot because there's a hard cutover to a repo that never had the exposure in the first place.

**One free-tier gotcha discovered along the way:** GitHub Pages requires a **public** source repo on the Free plan — Pro is needed to serve Pages from a private one. Making a repo private to fix a data-exposure problem takes the live site down as an immediate, unavoidable side effect on Free. (Cloudflare Pages, by contrast, deploys from a private GitHub repo on its own free tier without issue — worth knowing as an alternative if keeping one repo private *and* live matters more than a clean relaunch.)

**Standing takeaway:** never upload a real personal-data export to a repo — public *or* private — as a "let me just show you the file" reference. Read it, extract only the structural facts needed (column names, shape), and delete it in the same breath, before it's committed if at all possible. A private repo still has every collaborator with access, still has Actions runners that read the file into their working directory during a run, and still means the data exists in git history somewhere — private only narrows who could see it, it doesn't change what "the file is now in the repo" means structurally.
