<script>
	...
	import FinancialHeadlines from '../components/FinancialHeadlines.svelte';
	...
	// Read-only, exactly like `growthRate` above: #172's lens toggle only needs `partner.name` to
	// label the "Partner" option, and entering or editing a partner is the Settings tab's job (#170).
	/** @type {import('$lib/types.js').Partner | null} */
	let partner = $state(null);

	// Read-only against the store, same convention as `growthRate`/`partner` above: the Financial
	// Headlines card (#264) reads `profile.retirement_target` for its FIRE progress headline, but
	// entering or editing a profile is the Settings tab's job.
...
<div class="mt-6 flex max-w-2xl flex-col gap-6">
	<FinancialHeadlines {monthlyEntries} {profile} />
	<NetWorthChart {monthlyEntries} {partner} {growthRate} />
