<script>
	...
	import AssetTracker from '../../components/AssetTracker.svelte';
	import { appData, getPersistenceMode, hydrateAppData, syncState } from '$lib/index.js';

	// `assets` maps onto `AppData.assets` directly, so the store (#5) owns it — hydrated from it
	// on mount below, and — once `ready` — every local change is written back into the store,
	// whose own debounced sync then persists it to the Gist (or the localStorage fallback).
	// `ready` guards against the pre-hydrate `[]` this starts as overwriting whatever was actually
	// stored, the same pattern the property/pensions/dividends tabs' `+page.svelte` use for their
	// own lists.
...
<p>
	...
	expected annual change and annual holding cost, plus each asset's gain/loss, annualised CAGR, net
	position after holding costs, a future value projection and a toggle for whether it counts towards
	net worth.
</p>
