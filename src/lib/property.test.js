...
describe('propertyPortfolioSummary', () => {
	...
	it('adds offset savings on top of equity for what counts towards net worth, without changing equity itself', () => {
		const property = createProperty({
			value: 350_000,
			mortgage_balance: 139_339.1,
			mortgage_type: 'offset',
			offset_savings_balance: 65_148.85,
			include_in_net_worth: true
		});
		const summary = propertyPortfolioSummary([property]);

		expect(summary.totalEquity).toBe(210_660.9);
		expect(summary.totalOffsetSavings).toBe(65_148.85);
		expect(summary.includedInNetWorth.equity).toBe(210_660.9);
		expect(summary.includedInNetWorth.offsetSavings).toBe(65_148.85);
		expect(summary.includedInNetWorth.netWorthValue).toBe(275_809.75);
	});

	it('leaves offset savings out of the totals when the property is excluded from net worth', () => {
		const property = createProperty({
			value: 350_000,
			mortgage_balance: 139_339.1,
			mortgage_type: 'offset',
		...
		// Still counted in the unconditional totals (mirrors totalEquity's own "every property,
		// regardless of the toggle" rule) -- only the includedInNetWorth/excludedFromNetWorth split
		// respects the toggle.
		expect(summary.totalOffsetSavings).toBe(65_148.85);
		expect(summary.includedInNetWorth.offsetSavings).toBe(0);
		expect(summary.excludedFromNetWorth.offsetSavings).toBe(65_148.85);
	});
});
...
describe('propertyOffsetSavings', () => {
	it('is the recorded balance when the mortgage type is offset', () => {
		expect(
			propertyOffsetSavings(
				createProperty({ mortgage_type: 'offset', offset_savings_balance: 65_148.85 })
			)
		).toBe(65_148.85);
	});

	it('is 0 for every other mortgage type, even with a stray non-zero balance on record', () => {
		expect(
			propertyOffsetSavings(
				createProperty({ mortgage_type: 'fixed', offset_savings_balance: 65_148.85 })
			)
		).toBe(0);
		expect(
			propertyOffsetSavings(
				createProperty({ mortgage_type: 'tracker', offset_savings_balance: 65_148.85 })
			)
		).toBe(0);
	});

	it('is tolerant of a missing or malformed property rather than throwing', () => {
		expect(propertyOffsetSavings(undefined)).toBe(0);
		expect(propertyOffsetSavings(null)).toBe(0);
		expect(propertyOffsetSavings({})).toBe(0);
	});
});
...
describe('propertyEquityProjection', () => {
	...
	it('an offset mortgage amortises faster than the same rate/payment without one', () => {
		const withoutOffset = createProperty({
			value: 300_000,
			mortgage_balance: 200_000,
			interest_rate: 4,
			monthly_payment: 1_500,
			mortgage_type: 'fixed',
			offset_savings_balance: 50_000,
			growth_rate: 0
		});
		const withOffset = createProperty({
			...withoutOffset,
			mortgage_type: 'offset'
		});

		const balanceWithoutOffset = propertyEquityProjection(withoutOffset, 5)[5].mortgageBalance;
		const balanceWithOffset = propertyEquityProjection(withOffset, 5)[5].mortgageBalance;

		// The savings balance isn't recorded (0) unless mortgage_type is 'offset', so the same
		// figure entered against a 'fixed' mortgage above has no effect on the projection at all —
		// only relabelling it 'offset' brings the discount in.
		expect(balanceWithOffset).toBeLessThan(balanceWithoutOffset);
	});

	it('an offset savings balance bigger than the mortgage floors chargeable interest at zero, not negative', () => {
		const property = createProperty({
			value: 300_000,
			...
			monthly_payment: 1_500,
			mortgage_type: 'offset',
			offset_savings_balance: 500_000,
			growth_rate: 0
		});
		const points = propertyEquityProjection(property, 3);

		// No interest accrues at all once fully offset -- the balance simply pays down by the full
		// monthly payment each month, same as a 0% mortgage would.
		expect(points[1].mortgageBalance).toBeCloseTo(50_000 - 1_500 * 12, 0);
	});
});
...
describe('amortiseMortgageMonth', () => {
	it('defaults offsetBalance to 0 — every pre-existing caller is unaffected', () => {
		expect(amortiseMortgageMonth(200_000, 0.04 / 12, 1_500)).toBe(
			amortiseMortgageMonth(200_000, 0.04 / 12, 1_500, 0)
		);
	});

	it('charges interest on balance minus offsetBalance', () => {
		const withOffset = amortiseMortgageMonth(200_000, 0.06 / 12, 1_500, 50_000);
		const withoutOffset = amortiseMortgageMonth(200_000, 0.06 / 12, 1_500, 0);
		expect(withOffset).toBeLessThan(withoutOffset);
	});

	it('floors chargeable balance at zero when offsetBalance exceeds balance', () => {
		// No interest at all: the whole payment goes straight to principal.
		expect(amortiseMortgageMonth(50_000, 0.06 / 12, 1_500, 500_000)).toBe(48_500);
	});
});
...
describe('propertyProjectionYearsForTargetAge', () => {
	const now = new Date('2026-08-07T00:00:00Z');

	it('is PROPERTY_PROJECTION_DEFAULT_TARGET_AGE 80', () => {
		expect(PROPERTY_PROJECTION_DEFAULT_TARGET_AGE).toBe(80);
	});

	it('is targetAge minus current age, for a birthday already reached this year', () => {
		// dob_month 8 (August): the birthday has already passed by 7 August, so age is exact.
		expect(propertyProjectionYearsForTargetAge(66, 1990, 8, now)).toBe(30);
	});

	it("subtracts a year when this year's birthday has not been reached yet", () => {
		// dob_month 12 (December): the birthday hasn't happened yet by 7 August, so age is one
		// less than the naive year difference.
		expect(propertyProjectionYearsForTargetAge(65, 1990, 12, now)).toBe(30);
	});

	it('falls back to year-only arithmetic when dob_month is not recorded', () => {
		expect(propertyProjectionYearsForTargetAge(70, 1990, null, now)).toBe(34);
		expect(propertyProjectionYearsForTargetAge(70, 1990, undefined, now)).toBe(34);
	});

	it('returns null when dobYear is not recorded — there is no age to work from', () => {
		expect(propertyProjectionYearsForTargetAge(80, null, 8, now)).toBeNull();
		expect(propertyProjectionYearsForTargetAge(80, undefined, 8, now)).toBeNull();
	});

	it('clamps a target age at or below current age to 0, not a negative horizon', () => {
		expect(propertyProjectionYearsForTargetAge(36, 1990, 8, now)).toBe(0);
		expect(propertyProjectionYearsForTargetAge(20, 1990, 8, now)).toBe(0);
	});

	it("clamps an absurdly high target age to 100, matching propertyEquityProjection's own cap", () => {
		expect(propertyProjectionYearsForTargetAge(9_999, 1990, 8, now)).toBe(100);
	});

	it("feeds straight into propertyEquityProjection's own years horizon", () => {
		const property = { value: 300_000, mortgage_balance: 200_000, growth_rate: 0 };
		const years = propertyProjectionYearsForTargetAge(66, 1990, 8, now) ?? 0;
		expect(propertyEquityProjection(property, years)).toHaveLength(years + 1);
	});
});
