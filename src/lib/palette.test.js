...
describe('chartSeriesColor', () => {
	it('hands out the slots in order, one per series index', async () => {
		const { chartSeriesColor, CHART_SERIES_COUNT } = await import('./palette.js');
		expect(chartSeriesColor(0)).toBe('hsl(var(--chart-1))');
		expect(chartSeriesColor(2)).toBe('hsl(var(--chart-3))');
		expect(chartSeriesColor(CHART_SERIES_COUNT - 1)).toBe(
			`hsl(var(--chart-${CHART_SERIES_COUNT}))`
		);
	});

	it('wraps back to the first slot past the last one, rather than running out', async () => {
		const { chartSeriesColor, CHART_SERIES_COUNT } = await import('./palette.js');
		expect(chartSeriesColor(CHART_SERIES_COUNT)).toBe(chartSeriesColor(0));
		expect(chartSeriesColor(CHART_SERIES_COUNT + 2)).toBe(chartSeriesColor(2));
		expect(chartSeriesColor(CHART_SERIES_COUNT * 4 + 1)).toBe(chartSeriesColor(1));
	});

	it('names a slot the stylesheet actually defines, for any index at all', async () => {
		const { chartSeriesColor, CHART_SERIES_COUNT } = await import('./palette.js');
		for (let index = 0; index < CHART_SERIES_COUNT * 3 + 1; index++) {
			const slot = Number(
				/** @type {RegExpMatchArray} */ (chartSeriesColor(index).match(/--chart-(\d+)/))[1]
			);
			expect(slot).toBeGreaterThanOrEqual(1);
			expect(slot).toBeLessThanOrEqual(CHART_SERIES_COUNT);
		}
	});

	it('falls back to the first slot for input that is not a series index', async () => {
		const { chartSeriesColor } = await import('./palette.js');
		// A negative, fractional, NaN or non-numeric index is a caller bug; `hsl(var(--chart-NaN))`
		// would paint the series black with nothing in the console to explain it.
		for (const index of [-1, -6, 1.5, NaN, Infinity]) {
			expect(chartSeriesColor(index)).toBe('hsl(var(--chart-1))');
		}
		expect(chartSeriesColor(/** @type {never} */ ('2'))).toBe('hsl(var(--chart-1))');
		expect(chartSeriesColor(/** @type {never} */ (undefined))).toBe('hsl(var(--chart-1))');
	});

	it('counts at least the five slots the wider palette added', async () => {
		const { CHART_SERIES_COUNT } = await import('./palette.js');
		expect(CHART_SERIES_COUNT).toBeGreaterThanOrEqual(5);
	});
});
