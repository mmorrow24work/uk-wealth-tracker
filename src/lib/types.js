 ...
 * @property {string | null} ticker Ticker symbol for live price lookup (e.g. "VWRL.L" or "AAPL").
 *   Null means not linked to a price feed.
 * @property {number | null} last_price Per-share/unit price quoted at the last live price refresh,
 *   in whatever currency and unit the exchange quotes that ticker in (pounds, pence, dollars — the
 *   feed doesn't say, and nothing here needs to know). It exists so a later quote can be turned
 *   into a *ratio*, `newPrice / last_price`, which is unit-free and can scale `value` without a
 *   share count. Null until the first successful fetch, and reset to null by any manual edit that
 *   breaks the price↔value pairing — see `model.js`'s `carryLastPrice`.
 */
 ...
 * @property {number} offset_savings_balance Linked savings balance (£) offsetting the mortgage —
 *   only meaningful when `mortgage_type` is `'offset'`; interest accrues on
 *   `mortgage_balance - offset_savings_balance` (floored at 0) instead of the full balance. `0`
 *   for every other mortgage type, where it has no effect.
 ...
 * @property {number} purchase_price Original purchase price (£), for capital gains tax calculation.
 *   Default `0`; `null` means not recorded.
 * @property {string | null} purchase_date Date of purchase, ISO `YYYY-MM-DD`. Needed for CGT
 *   holding period. Null means not recorded.
 * @property {string | null} let_from Date the property started being let out, ISO `YYYY-MM-DD`.
 *   For Private Residence Relief pro-rating in CGT: `null` means "never let" (for a primary
 *   residence/rented residence, lived in throughout ownership) or "let for the whole ownership
 *   period" (for a buy-to-let/holiday home with no lived-in period). The CGT engine will
 *   interpret this alongside `type`.
...
/**
 * A life insurance policy. Tracked separately from investments/pensions since it has distinct
 * characteristics: a death benefit (not a portfolio value), an in-trust flag for IHT planning,
 * and the provider/policy relationship. Can count towards the estate value for IHT purposes.
 *
 * @typedef {object} LifeInsurance
 * @property {string} id
 * @property {string} name Policy name or description, e.g. "Level-term £500k to age 60".
 * @property {string} provider Insurance company name, e.g. "Zurich", "Royal London".
 * @property {number} sum_assured Death benefit (£).
 * @property {number} current_value Current surrender value or cash-in value (£).
 * @property {boolean} in_trust Whether the policy is held in trust for IHT planning purposes.
 * @property {string} notes Free text.
 */
 ...
 * @property {LifeInsurance[]} life_insurance
