 ...
 *    `MortgageType` (`types.js`) has no `'interest_only'` value to check for. What makes a
 *    mortgage interest-only is structural, not a label: its recorded `monthly_payment` is, by
 *    definition, exactly the interest on its balance — which is precisely the boundary condition
 *    convention 1 already treats as "never clears". Plugging one into
 *    {@link remainingMortgageTermMonths} therefore already returns `null` without this module
 *    reading `mortgage_type` at all (an underwater mortgage on any recorded type hits the same
 *    boundary the same way, `offset`'s own reduced-interest effect included — a `Property`'s
 *    `interest_rate`/`monthly_payment` here are read as recorded, not re-derived net of an offset
 *    balance, so this module already sees whatever rate an offset mortgage's owner would enter),
 *    and every function that would otherwise need a
