...
/**
 * {@link COMMIT_SHA}'s own commit timestamp, ISO 8601 (e.g. "2026-08-12T14:03:21+01:00") --
 * `__COMMIT_DATE__` is a build-time string replacement (vite.config.js's `commitDate()`), same
 * mechanism as `COMMIT_SHA`. Falls back to "" outside a Vite build, same as `COMMIT_SHA` falls
 * back to "dev" -- empty rather than "dev" here because there's no sensible placeholder date, and
 * callers already need to handle a missing commit (see the footer's existing `COMMIT_SHA` guard).
 * @type {string}
 */
