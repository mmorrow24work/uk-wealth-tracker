 ...
 *    point per year.
 * 5. **How far out is "until I turn 80"?** {@link propertyProjectionYearsForTargetAge} — turns a
 *    target age into the `years` horizon {@link propertyEquityProjection} takes, so a chart control
 *    can offer "project until age…" without the projection itself knowing what an age is.
 *
...
/**
 * A property's offset savings balance — but only when it is actually attached to an offset
 * mortgage. `offset_savings_balance` has no effect on anything for any other `mortgage_type`,
 * even a stray non-zero value left over from switching a property's mortgage type away from
 * `'offset'`, the same defensive gate {@link propertyEquityProjection} applies for the interest
 * calculation itself.
 *
 * Deliberately kept separate from {@link propertyEquity} rather than folded into it: `equity`
 * means "value minus what's owed on the property" everywhere else it's read (the row display, the
 * 30-year projection, `budget-policy.js`'s residence nil-rate band calculation, which specifically
 * needs the home's own net value and nothing else) — an offset savings pot is genuinely owned
 * money, but it isn't property equity, so callers that want "what should count towards net worth"
 * add this on top explicitly instead of `propertyEquity` silently changing meaning under them.
 *
 * @param {Partial<import('./types.js').Property> | null} [property]
 * @returns {number} (£)
 */
export function propertyOffsetSavings(property) {
	if (property?.mortgage_type !== 'offset') return 0;
	return roundMoney(asMoney(property?.offset_savings_balance));
}
 ...
 * @property {number} equity (£) — value minus mortgage, {@link propertyEquity} summed. Does not
 *   include `offsetSavings` below — see {@link netWorthValue} for the figure that does.
 * @property {number} offsetSavings (£) — {@link propertyOffsetSavings} summed.
 * @property {number} netWorthValue (£) — `equity + offsetSavings`, the actual "what should count
 *   towards net worth" figure for this slice.
 */
...
function slice(properties) {
	const equity = roundMoney(properties.reduce((total, p) => total + propertyEquity(p), 0));
	const offsetSavings = roundMoney(
		properties.reduce((total, p) => total + propertyOffsetSavings(p), 0)
	);
	...
	return {
		...
		equity,
		offsetSavings,
		netWorthValue: roundMoney(equity + offsetSavings)
	};
}
 ...
 * @property {number} totalOffsetSavings (£) — {@link propertyOffsetSavings} summed across every
 *   property, regardless of the toggle — mirrors `totalEquity`.
 * @property {PropertySlice} includedInNetWorth Properties whose equity (and any offset savings)
 *   counts towards net worth.
...
export function propertyPortfolioSummary(properties) {
	...
	return {
		...
		totalOffsetSavings: roundMoney(list.reduce((total, p) => total + propertyOffsetSavings(p), 0)),
 ...
 *
 * `offsetBalance` (default `0`, every existing caller unaffected) models an offset mortgage:
 * interest accrues on `balance - offsetBalance` instead of the full balance, floored at zero
 * rather than letting a savings pot bigger than the mortgage earn negative interest. The savings
 * pot itself is not projected to grow or shrink here — held constant, the same "today's recorded
 * fields, no projection of the input itself" treatment every other field in this module gets.
 *
 ...
 * @param {number} [offsetBalance] Savings balance offsetting the interest-bearing balance (£).
 * @returns {number} Closing balance (£), floored at zero.
 */
export function amortiseMortgageMonth(balance, monthlyRate, payment, offsetBalance = 0) {
	if (balance <= 0) return 0;
	const chargeableBalance = Math.max(0, balance - offsetBalance);
	const interest = chargeableBalance * monthlyRate;
...
/**
 * Default "project until age…" a chart's target-age control starts on — a round planning horizon
 * well past a typical mortgage term, not anything derived from the user's own age.
 */
export const PROPERTY_PROJECTION_DEFAULT_TARGET_AGE = 80;
 ...
 *
 * **Offset mortgages** (`mortgage_type === 'offset'`) accrue interest on
 * `mortgage_balance - offset_savings_balance` each month instead of the full balance — see
 * {@link amortiseMortgageMonth}. The savings pot is held constant across the projection, not
 * grown or spent down, the same snapshot treatment `growth_rate`/`interest_rate` themselves get:
 * this projects what today's fields imply, not a forecast of the savings pot's own future.
 *
 * @param {Partial<import('./types.js').Property> | null} [property]
...
export function propertyEquityProjection(property, years = PROPERTY_PROJECTION_YEARS) {
	...
	const offsetBalance =
		property?.mortgage_type === 'offset' ? asMoney(property?.offset_savings_balance) : 0;
...
/* -------------------------------------------------------------------------- */
/* Target age -> projection horizon                                          */
/* -------------------------------------------------------------------------- */

/**
 * Turn "project until age `targetAge`" into the `years` horizon {@link propertyEquityProjection}
 * takes. `null` when there is no `dobYear` to work an age out from — the same explicit
 * "unavailable" convention `milestones.js`'s `retirementMarker` uses, so a caller falls back to
 * {@link PROPERTY_PROJECTION_YEARS} and says so, rather than this function silently guessing an age.
 *
 * Reuses {@link ageAtPoint} for the DOB-to-age arithmetic — the same helper `age-filter.js` reuses
 * rather than reimplementing age math a second time.
 *
 * The result is clamped to 0…{@link MAX_PROJECTION_YEARS}, the same window
 * {@link propertyEquityProjection} already clamps `years` to internally: a target age at or below
 * today's age produces `0` rather than a negative horizon, and an absurdly high target age is
 * capped rather than building a runaway series.
 *
 * @param {number} targetAge
 * @param {number | null | undefined} dobYear
 * @param {number | null | undefined} [dobMonth]
 * @param {Date} [now] Defaults to the real clock; pass a fixed `Date` for a repeatable answer.
 * @returns {number | null}
 */
export function propertyProjectionYearsForTargetAge(
	targetAge,
	dobYear,
	dobMonth,
	now = new Date()
	...
	const currentAge = ageAtPoint(dobYear, dobMonth ?? null, {
		year: now.getFullYear(),
		month: now.getMonth() + 1
	});

	return Math.min(MAX_PROJECTION_YEARS, Math.max(0, targetAge - currentAge));
}
