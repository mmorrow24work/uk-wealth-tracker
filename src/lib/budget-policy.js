...
import { DEBT_TYPES, DEBT_TYPE_LABELS, WRAPPERS, WRAPPER_LABELS } from './enums.js';
import {
	NIL_RATE_BAND,
...
import { propertyEquity, propertyOffsetSavings } from './property.js';
 ...
 * @property {readonly Partial<import('./types.js').LifeInsurance>[]} [lifeInsurance] Recorded
 *   policies. Unlike every other list here, a policy's own `in_trust` flag — not an
 *   `include_in_net_worth` toggle — decides whether it counts; see {@link payableIntoEstate}.
 */
...
/**
 * Whether one life insurance policy's sum assured is chargeable to the estate.
 *
 * The rule is binary and has no partial case: a policy written **in trust** pays its beneficiaries
 * directly and never becomes part of the deceased's estate, so it is chargeable to nothing however
 * large the cover; a policy **not** in trust pays into the estate, and its whole sum assured is
 * chargeable at the estate's own rate. This is the single most valuable thing this app can say about
 * life cover, and also the easiest to get backwards — a £500,000 policy is either wholly in or
 * wholly out of a bill that runs at 40% above the bands.
 *
 * Keyed on `in_trust !== true` rather than `=== false`, so a missing, `undefined` or malformed flag
 * counts the policy *into* the estate: the same "treat missing as the more conservative case"
 * reading `model.js`'s own `asBoolean(source.in_trust, false)` takes when a document is loaded, and
 * the safer of the two errors here. Overstating an estate quotes a bill that turns out not to be
 * owed; understating it hides one that is.
 *
 * Sum assured, not `current_value`: the estate is credited with what the policy *pays out* on death,
 * which is the point of the cover — the surrender value is what it would fetch if cashed in while
 * alive, a different question this valuation is not asking.
 *
 * @param {Partial<import('./types.js').LifeInsurance> | undefined | null} policy
 * @returns {boolean}
 */
function payableIntoEstate(policy) {
	return policy?.in_trust !== true;
}

/**
 * @param {readonly Partial<import('./types.js').LifeInsurance>[]} policies
 * @returns {number} Their sums assured added (£).
 */
function sumAssured(policies) {
	return policies.reduce((total, policy) => total + asNumber(policy?.sum_assured, 0), 0);
}
 ...
 *
 * Deliberately `propertyEquity(home)` alone, not `+ propertyOffsetSavings(home)` — the residence
 * band is specifically about the home's own net value, not the deceased's cash generally, so an
 * offset mortgage's linked savings pot (real money, but not part of *the home*) stays out of this
 * figure even though {@link estateValuation} below does add it to the wider estate total.
 *
...
/**
 * One `Wrapper`'s slice of `investments` — the row an estate breakdown draws, in `WRAPPERS`
 * order, zero-value wrappers omitted, the same "don't show a computed line for data that isn't
 * there" rule `EstateSummary.svelte` already applies to Lifetime ISA pots and Offset mortgage
 * savings.
 *
 * @typedef {object} EstateInvestmentBreakdownRow
 * @property {import('./enums.js').Wrapper} wrapper
 * @property {string} label
 * @property {number} amount (£) Grown to this tax year, same as `investments` — convention 6.
 */

/**
 * One `DebtType`'s slice of `debts` — the row an estate breakdown draws, in `DEBT_TYPES` order,
 * zero-balance types omitted, the same "don't show a computed line for data that isn't there"
 * rule `EstateInvestmentBreakdownRow` follows for `investmentsByWrapper`. Held flat, like `debts`
 * itself — convention 6 — so unlike that sibling row this one is never `grown()`.
 *
 * @typedef {object} EstateDebtBreakdownRow
 * @property {import('./enums.js').DebtType} type
 * @property {string} label
 ...
 * @property {EstateInvestmentBreakdownRow[]} investmentsByWrapper `investments` broken down by
 *   `Wrapper` — a breakdown of that same figure, not a second one, so its `amount`s always sum
 *   back to `investments`. Honours `exclude_from_net_worth` exactly as `investments` does, since
 *   each row is `sumInvestmentValues` run on that wrapper's holdings alone, grown the same way.
 * @property {number} propertyEquity Counted property equity (£) — value minus mortgage, no offset
 *   savings included (see `offsetSavings` below for that; kept as a separate field for the same
 *   reason {@link import('./property.js').propertyOffsetSavings}'s own doc gives).
 * @property {number} offsetSavings Savings balances linked to a counted property's offset
 *   mortgage (£) — genuinely owned money, so it belongs in the estate even though it isn't
 *   `propertyEquity`. Excluded from `residence` (the residence nil-rate band's own figure) —
 *   see {@link qualifyingResidenceValue}.
 * @property {number} physicalAssets Counted physical assets (£).
 * @property {number} lifeInsurance Sum assured of the recorded policies that are **not** written in
 *   trust (£) — the cover that pays into the estate and is chargeable, per
 *   {@link payableIntoEstate}. Grown to this tax year like every other asset-side component
 *   (convention 6), and, like `offsetSavings`, deliberately outside `residence`: cover is money in
 *   the estate, but it is not part of *the home*, so it must never inflate the residence nil-rate
 *   band.
 * @property {number} lifeInsuranceInTrust Sum assured of the recorded policies that **are** written
 *   in trust (£) — reported so a caller can say how much cover is excluded and why, never added to
 *   `withoutPensions`/`total`. The same always-reported/only-sometimes-counted shape
 *   `pensionPots`/`pensionsCounted` uses, and for the same reason: the exclusion is a fact about the
 *   estate worth showing, not an absence to stay silent about. Keeping it here rather than leaving a
 *   caller to re-sum the raw policy list is what holds the in-trust rule in one place.
 ...
 * @property {EstateDebtBreakdownRow[]} debtsByType `debts` broken down by `DebtType` — a
 *   breakdown of that same figure, not a second one, so its `amount`s always sum back to `debts`.
 *   Honours `exclude_from_net_worth` exactly as `debts` does, since each row is
 *   `sumDebtBalances` run on that type's debts alone.
 ...
 * (convention 7).
 *
 * Life insurance is split the same always-report/sometimes-count way, but on a rule that is about
 * the policy rather than the year: cover not written in trust is in `lifeInsurance` and inside
 * `total`, cover written in trust is in `lifeInsuranceInTrust` and inside nothing — see
 * {@link payableIntoEstate} for the rule and why a missing flag counts the cover in.
 *
 * @param {EstatePosition} [position]
...
export function estateValuation(position = {}, startYear = FIRST_MODELLED_TAX_YEAR, policy = {}) {
	...
	const investmentsByWrapper = WRAPPERS.map((wrapper) => {
		const amount = grown(
			sumInvestmentValues((position.investments ?? []).filter((inv) => inv?.wrapper === wrapper))
		);
		return amount === 0 ? null : { wrapper, label: WRAPPER_LABELS[wrapper], amount };
	}).filter((row) => row !== null);
	const equity = grown(
		countedProperties(position).reduce((total, property) => total + propertyEquity(property), 0)
	);
	const offsetSavings = grown(
		countedProperties(position).reduce(
			(total, property) => total + propertyOffsetSavings(property),
			0
		)
	);
	...
	const policies = position.lifeInsurance ?? [];
	const lifeInsurance = grown(sumAssured(policies.filter((policy) => payableIntoEstate(policy))));
	const lifeInsuranceInTrust = grown(
		sumAssured(policies.filter((policy) => !payableIntoEstate(policy)))
	);
	...
	const debtsByType = DEBT_TYPES.map((type) => {
		const amount = roundMoney(
			sumDebtBalances((position.debts ?? []).filter((debt) => debt?.type === type))
		);
		return amount === 0 ? null : { type, label: DEBT_TYPE_LABELS[type], amount };
	}).filter((row) => row !== null);
	...
	const withoutPensions = roundMoney(
		investments + equity + offsetSavings + physicalAssets + lifeInsurance + lifetimeIsaPots - debts
	);
