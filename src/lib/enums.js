 ...
 * order: Stocks ISA, SIPP, Shares, Crypto, Cash, Emergency Fund, Dividends, Property). `other` is
 * a later catch-all addition (issue #250), the same pattern `DEBT_TYPES`/`ASSET_CATEGORIES`
 * already follow.
 * @typedef {'stocks_isa' | 'sipp' | 'shares' | 'crypto' | 'cash' | 'emergency_fund' | 'dividends' | 'property' | 'other'} InvestmentType
 */
 ...
 * (issue #31), which owns the single `type: 'state'` record in `pensions[]`.
 * @type {readonly PensionType[]}
 */
...
/**
 * Property type (README.md → "Property Tracker"). `rented_residence` is a home the user lives in
 * as a tenant, not an owner — there is no mortgage or equity to record, so `value`/
 * `mortgage_balance`/etc. are typically left at their `0` defaults for one of these; the rent
 * itself is expected to be tracked as a recurring Budget expense instead, not duplicated here.
 * @typedef {'primary_residence' | 'buy_to_let' | 'holiday_home' | 'rented_residence'} PropertyType
 */
...
export const PROPERTY_TYPE_LABELS = Object.freeze({
	...
	rented_residence: "Rented (I'm the tenant)"
});

/**
 * Mortgage product type. `variable` is a lender's discount-variable deal — neither tracking the
 * Bank of England base rate (`tracker`) nor sitting at the lender's own standard variable rate
 * with no discount (`svr`). `offset` links a savings balance (`Property.offset_savings_balance`)
 * against the mortgage: interest only accrues on the balance net of that savings pot — see
 * `property.js`'s {@link import('./property.js').amortiseMortgageMonth}.
 * @typedef {'fixed' | 'tracker' | 'svr' | 'variable' | 'offset' | 'none'} MortgageType
 */

/** @type {readonly MortgageType[]} */
export const MORTGAGE_TYPES = Object.freeze([
	'fixed',
	'tracker',
	'svr',
	'variable',
...
export const MORTGAGE_TYPE_LABELS = Object.freeze({
	...
	variable: 'Variable rate (discount)',
	offset: 'Offset',
	none: 'No mortgage'
});
