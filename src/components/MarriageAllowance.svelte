<script>
	 ...
	 *
	 * `partner` (issue #146) is the household's real `Partner` record from the data model, where one
	 * is set up — `TaxCalculator` seeds the shared `partnerIncome` field from `partner.gross_salary`
	 * before this component ever renders, so a household with a partner profile opens on their actual
	 * salary rather than the £0 that used to force re-entering it here. There is still no marital
	 * status anywhere in the data model (README.md's "Partner profile" fields are DOB, retirement age,
	 * salary, pension, NI years — nothing about marriage), so the "we're married" checkbox stays a
	 * manual fact the user states regardless of whether a partner profile exists; `partner` only
	 * changes where the *income* figure comes from and what the field's helper text says about it,
	 * falling back to today's fully-manual entry when it is `null`. Nothing about the maths in
	 * `$lib/marriage-allowance.js` changes — this is a data-source change to one input, not a new
	 * calculation.
	 */
	import {
		higherRateThreshold,
	...
	/** The partner's real first name, where the profile has one — `null` blends back to "your partner". */
	const partnerName = $derived(partner && partner.name.trim() !== '' ? partner.name.trim() : null);
		...
		{#if married}
			<div class="flex flex-col gap-1">
				...
				<span class="text-xs text-muted-foreground">
					{#if partner}
						Pre-filled from {partnerName ?? 'your partner'}'s profile ({formatMoney(
							partner.gross_salary
						)}) — change it here for a what-if, nothing is written back.
					{:else}
						No partner profile set up yet, so this is typed here each visit — add one on Settings to
						pre-fill it.
					{/if}
				</span>
			</div>
