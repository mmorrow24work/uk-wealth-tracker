...
describe('MarriageAllowance', () => {
	...
	it('names whose profile the partner income was pre-filled from (#146)', () => {
		const body = text({
			income: 40_000,
			married: true,
			partnerIncome: 5_000,
		...
		expect(body).toContain("Pre-filled from Alex's profile (£5,000)");
		expect(body).not.toContain('No partner profile set up yet');
	});

	it('falls back to the unnamed "your partner" wording when the partner profile has no name', () => {
		const body = text({
			income: 40_000,
			married: true,
			partnerIncome: 5_000,
			partner: createPartner({ gross_salary: 5_000 })
		});

		expect(body).toContain("Pre-filled from your partner's profile (£5,000)");
	});

	it('falls back to the fully-manual entry note when no partner profile is recorded', () => {
		const body = text({ income: 40_000, married: true, partnerIncome: 5_000 });

		expect(body).toContain('No partner profile set up yet');
		expect(body).not.toContain('Pre-filled from');
	});
