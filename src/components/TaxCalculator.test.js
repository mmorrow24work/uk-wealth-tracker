...
describe('TaxCalculator', () => {
	...
	it('seeds the shared partner-income field from the real partner profile where one exists (#146)', () => {
		// `partnerIncome` is shared with `bind:` between Child Benefit and Marriage Allowance; Child
		// Benefit's own field is always rendered (Marriage Allowance's is gated behind its "married"
		// checkbox), so its `value` attribute is the simplest place to see what the shared field
		// opened on.
		const withPartner = render(TaxCalculator, {
			props: {
				...profile({ gross_salary: 40_000 }),
				partner: createPartner({ name: 'Alex', gross_salary: 5_000 })
			}
		}).body;
		expect(withPartner).toContain(
			'id="cb-partner-income" type="number" min="0" step="500" value="5000"'
		);

		const withoutPartner = render(TaxCalculator, { props: profile({ gross_salary: 40_000 }) }).body;
		expect(withoutPartner).toContain(
			'id="cb-partner-income" type="number" min="0" step="500" value="0"'
		);
	});
