...
import { createDebt, createInvestment, createMonthlyEntry, createPartner } from '$lib/model.js';
...
describe('NetWorthChart month selector', () => {
	/** @returns {import('$lib/types.js').MonthlyEntry[]} */
	function history() {
		return [
			entry(1, 2026, { investments: [10_000] }),
			entry(2, 2026, { investments: [40_000] }),
			entry(3, 2026, { investments: [46_000] })
		];
	}

	it('offers no selector for a single recorded month', () => {
		const rendered = text({
			monthlyEntries: [entry(3, 2026, { investments: [12_500] })],
			showForecast: false
		});

		expect(rendered).not.toContain('Show figures for');
	});

	it('lists every recorded month, most recent first, defaulting to the latest', () => {
		const rendered = body({ monthlyEntries: history(), showForecast: false });

		expect(rendered).toContain('Show figures for');
		const options = [...rendered.matchAll(/<option[^>]*>([^<]+)</g)].map((match) => match[1]);
		expect(options).toEqual(['Mar 2026', 'Feb 2026', 'Jan 2026']);
		// Svelte marks the bound option as selected in the server render — same mechanism the horizon
		// picker's own test above relies on.
		expect(rendered).toMatch(/<option[^>]*selected[^>]*>Mar 2026/);
	});

	it('leaves the headline on the latest month when untouched, matching pre-#260 behaviour', () => {
		const rendered = text({ monthlyEntries: history(), showForecast: false });

		expect(rendered).toContain('£46,000');
		expect(rendered).toContain('at Mar 2026');
	});

	it('moves the headline figure to the month selected via initialSelectedMonth', () => {
		// The table fallback below always lists every recorded month's figures (including Mar's
		// £46,000) regardless of selection, so this asserts the *headline* — the number right after
		// the selector, before the table — rather than searching the whole render for £46,000's absence.
		const rendered = text({
			monthlyEntries: history(),
			showForecast: false,
			initialSelectedMonth: { month: 2, year: 2026 }
		});
		const headline = rendered.slice(0, rendered.indexOf('Show as a table'));

		expect(headline).toContain('£40,000');
		expect(headline).not.toContain('£46,000');
	});

	it('marks the selected month, not the latest, as selected in the rendered markup', () => {
		const rendered = body({
			monthlyEntries: history(),
			showForecast: false,
			initialSelectedMonth: { month: 2, year: 2026 }
		...
		expect(rendered).toMatch(/<option[^>]*selected[^>]*>Feb 2026/);
		expect(rendered).not.toMatch(/<option[^>]*selected[^>]*>Mar 2026/);
	});

	it('moves the caption to the selected month, keeping the recorded-since fact about the whole series', () => {
		const rendered = text({
			monthlyEntries: history(),
			showForecast: false,
			initialSelectedMonth: { month: 2, year: 2026 }
		...
		expect(rendered).toContain('at Feb 2026');
		// "3 months recorded since Jan 2026" is a fact about the whole series, not the selection — #288
		// leaves it alone even though the headline above it has moved.
		expect(rendered).toContain('3 months recorded since Jan 2026');
	});

	it('moves the month-on-month change to compare the selected month with the one before it', () => {
		const rendered = text({
			monthlyEntries: history(),
			showForecast: false,
			initialSelectedMonth: { month: 2, year: 2026 }
		...
		// Feb (£40,000) vs Jan (£10,000) — not Mar (£46,000) vs Feb (£40,000), the untouched comparison.
		expect(rendered).toContain('£30,000');
		expect(rendered).not.toContain('£6,000');
	});

	it('drops the month-on-month change when the selected month is the first recorded one', () => {
		const rendered = text({
			monthlyEntries: history(),
			showForecast: false,
			initialSelectedMonth: { month: 1, year: 2026 }
		});

		// Jan has no earlier month to compare against — the same "fewer than two points" null
		// `monthOnMonthChange` already returns for a series with only one recorded month.
		expect(rendered).not.toContain('month-on-month');
	});

	it('keeps the forecast anchored on the actual latest month regardless of selection', () => {
		const rendered = text({
			monthlyEntries: history(),
			initialSelectedMonth: { month: 1, year: 2026 }
		});

		expect(rendered).toContain('Projected from your Mar 2026 snapshot');
		expect(rendered).not.toContain('Projected from your Jan 2026 snapshot');
	});

	it('keeps the accessible summary describing the full recorded line, not the selection', () => {
		const rendered = body({
			monthlyEntries: history(),
			showForecast: false,
			initialSelectedMonth: { month: 1, year: 2026 }
		...
		expect(rendered).toContain('£10,000 in Jan 2026');
		expect(rendered).toContain('£46,000 in Mar 2026');
	});

	it('says nothing about the selection in the accessible summary when it matches the latest month', () => {
		const rendered = body({ monthlyEntries: history(), showForecast: false });

		expect(rendered).not.toContain('not the latest recorded month');
	});

	it('names the selected month in the accessible summary when it differs from the latest', () => {
		const rendered = body({
			monthlyEntries: history(),
			showForecast: false,
			initialSelectedMonth: { month: 2, year: 2026 }
		...
		expect(rendered).toContain('Showing Feb 2026, not the latest recorded month.');
	});
});
...
describe('NetWorthChart lens toggle', () => {
	/**
	 * One recorded month with a part-owned holding, so the household/you/partner totals genuinely
	 * differ — issue #172's toggle over #143's `ownership_pct` split.
	 *
	 * @returns {import('$lib/types.js').MonthlyEntry[]}
	 */
	function ownershipHistory() {
		return [
			createMonthlyEntry({
				month: 1,
				year: 2026,
				investments: [createInvestment({ value: 100_000, ownership_pct: 60 })]
			})
		];
	}

	it('renders all three lens options, defaulting to Household', () => {
		const rendered = body({ monthlyEntries: ownershipHistory(), showForecast: false });

		expect(rendered).toContain('role="radiogroup"');
		expect(rendered).toContain('aria-label="Net worth lens"');
		expect(rendered).toContain('Household');
		expect(rendered).toContain('You');
		expect(rendered).toContain('Partner');
	});

	it('shows the household total under the default lens, unchanged from before #172', () => {
		const rendered = text({ monthlyEntries: ownershipHistory(), showForecast: false });

		expect(rendered).toContain('£100,000');
	});

	it('scales the headline figure by ownership share under the you lens', () => {
		const rendered = text({
			monthlyEntries: ownershipHistory(),
			showForecast: false,
			initialLens: 'you'
		});
	...
	it('scales the headline figure by the remaining share under the partner lens', () => {
		const rendered = text({
			monthlyEntries: ownershipHistory(),
			showForecast: false,
			initialLens: 'partner'
	...
	it('labels the partner toggle with the recorded name', () => {
		const rendered = text({
			monthlyEntries: ownershipHistory(),
			showForecast: false,
			partner: createPartner({ name: 'Alex' })
		});

		expect(rendered).toContain('Alex');
	});

	it('falls back to the generic label when no partner is recorded', () => {
		const rendered = text({ monthlyEntries: ownershipHistory(), showForecast: false });

		expect(rendered).toContain('Partner');
	});

	it('hides the forecast control and explains why outside the household lens', () => {
		const rendered = text({ monthlyEntries: history(), initialLens: 'you' });

		expect(rendered).not.toContain('Show forecast');
		expect(rendered).not.toContain('Projected from your');
		expect(rendered).toContain('only shown on the Household lens');
	});

	it('keeps the forecast control under the household lens', () => {
		const rendered = text({ monthlyEntries: history(), initialLens: 'household' });

		expect(rendered).toContain('Show forecast');
		expect(rendered).toContain('Projected from your');
	});

	it('names whose net worth the accessible summary describes', () => {
		const household = body({ monthlyEntries: history() });
		const you = body({ monthlyEntries: history(), initialLens: 'you' });
		const namedPartner = body({
			monthlyEntries: history(),
			initialLens: 'partner',
			partner: createPartner({ name: 'Alex' })
		});
		const unnamedPartner = body({ monthlyEntries: history(), initialLens: 'partner' });

		expect(household).toContain('Household net worth.');
		expect(you).toContain('Your net worth.');
		expect(namedPartner).toContain("Alex's net worth.");
		expect(unnamedPartner).toContain("Partner's net worth.");
	});
