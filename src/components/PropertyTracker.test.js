...
import { createProfile, createProperty } from '$lib/model.js';
import { PROPERTY_PROJECTION_DEFAULT_TARGET_AGE } from '$lib/property.js';
...
describe('PropertyTracker', () => {
	...
	/* ---------------------------------------------------------------------- */
	/* "Until age" projection horizon control (#248)                          */
	/* ---------------------------------------------------------------------- */

	it('shows the "until age" control, defaulted to the target age default, when the profile has a birth year', () => {
		const { body } = render(PropertyTracker, {
			props: {
				properties: [createProperty({ name: '12 Oak Avenue', value: 300_000 })],
				profile: createProfile({ dob_year: 1990, dob_month: 6 })
			}
		});
		expect(body).toContain('until age');
		expect(body).toMatch(
			new RegExp(
				`<input[^>]*id="property-chart-target-age"[^>]*value="${PROPERTY_PROJECTION_DEFAULT_TARGET_AGE}"`
			)
		);
	});

	it('hides the "until age" control and explains why when the profile has no birth year', () => {
		const body = text({
			properties: [createProperty({ name: '12 Oak Avenue', value: 300_000 })],
			profile: createProfile({ dob_year: null })
		});
		expect(body).not.toContain('until age');
		expect(body).toContain('Add your date of birth on the forecast tab');
	});

	it('hides the "until age" control when no profile is passed at all', () => {
		const body = text({
			properties: [createProperty({ name: '12 Oak Avenue', value: 300_000 })]
		});
		expect(body).not.toContain('until age');
	...
	/* ---------------------------------------------------------------------- */
	/* Combined "all properties" chart toggle (#279)                          */
	/* ---------------------------------------------------------------------- */

	it('offers the single/all properties toggle once a property exists, defaulting to single', () => {
		const { body } = render(PropertyTracker, {
			props: { properties: [createProperty({ name: '12 Oak Avenue', value: 300_000 })] }
		});
		expect(body).toContain('role="radiogroup"');
		expect(body).toContain('aria-label="Property chart view"');
		expect(body).toContain('Single property');
		expect(body).toContain('All properties');
		expect(body).toMatch(/<input type="radio"[^>]*value="single"[^>]*checked/);
	});

	it('shows no toggle when there are no properties', () => {
		const body = text();
		expect(body).not.toContain('Property chart view');
	});

	it('shows the property picker and single-property chart in single mode', () => {
		const body = text({
			properties: [createProperty({ name: '12 Oak Avenue', value: 300_000, growth_rate: 4 })],
		...
		expect(body).toContain('Equity growth projection for');
		expect(body).toContain('12 Oak Avenue — equity projection');
		expect(body).not.toContain('All properties — equity projection');
	});

	it('hides the property picker and shows the combined chart in all-properties mode', () => {
		const body = text({
			properties: [
				createProperty({ name: 'Property A', value: 100_000, growth_rate: 3 }),
				createProperty({ name: 'Property B', value: 200_000, growth_rate: 3 })
			],
			initialChartMode: 'all'
		});
		expect(body).not.toContain('Equity growth projection for');
		expect(body).toContain('All properties — equity projection');
		expect(body).toContain('Property A');
		expect(body).toContain('Property B');
	});

	it('checks the "All properties" radio when seeded into that mode', () => {
		const { body } = render(PropertyTracker, {
			props: {
				properties: [createProperty({ name: '12 Oak Avenue', value: 300_000 })],
				initialChartMode: 'all'
		...
		expect(body).toMatch(/<input type="radio"[^>]*value="all"[^>]*checked/);
	});

	it('keeps the "until age" control alongside the toggle in both modes', () => {
		const single = text({
			properties: [createProperty({ name: '12 Oak Avenue', value: 300_000 })],
			profile: createProfile({ dob_year: 1990, dob_month: 6 }),
			initialChartMode: 'single'
		...
		expect(single).toContain('until age');
		expect(all).toContain('until age');
	});
});
