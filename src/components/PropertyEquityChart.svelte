...
<script>
	 ...
	 * `--destructive` rather than a third chart hue: this app has only `--chart-1`/`--chart-2`
	 * defined (`app.css`), and a shrinking debt is the one series here that reads naturally as "the
	 * bad one going down is good news" in a warning colour.
	 */
