<script>
	 ...
	 *
	 * `partnerIncome` is seeded from `partner.gross_salary` (issue #146) the same read-only, seed-once
	 * way `income`/`region`/`sacrifice` are seeded from `profile` above — real partner data pre-fills
	 * the field instead of opening on a blank £0, but nothing is written back, and the user can still
	 * override it for a what-if. `partner` is also passed straight through to `MarriageAllowance`,
	 * which needs to know *whether* a partner profile exists (not just its salary) to decide between
	 * that pre-filled figure and its existing fully-manual entry for a household with no partner
	 * recorded yet.
	 *
	 ...
	 * }}
	 */
	let { profile = createProfile(), partner = null } = $props();

	/** Shared between `ChildBenefitCharge` and `MarriageAllowance` — see the header note above. */
	// svelte-ignore state_referenced_locally
	let partnerIncome = $state(partner?.gross_salary ?? 0);
...
{#if incomeIsValid}
	...
	<MarriageAllowance income={adjustedNetIncome} {region} bind:partnerIncome {partner} />
