<script>
	 ...
	 *
	 * The one-off large costs overlay (#161) is the `OneOffCostsPanel` section at the bottom,
	 * alongside the other two: same wiring again, but for a lump sum taken *out* on a stated date —
	 * a wedding, a car, a home renovation — rather than a market move or a standing-order change.
	 * Unlike its two siblings it takes an editable list rather than fixed dials, since `$lib/one-off-
	 * costs.js` supports any number of costs landing on any number of dates.
	 *
	 * The mortgage rate rise overlay (#158, controls in #184) is the `MortgageRateRisePanel` section
	 * alongside the others: same baseline `Forecast`/position wiring, but per-property rather than
	 * whole-portfolio, so it also takes `properties` — the one prop none of the siblings need — to
	 * pick a real mortgaged property from rather than describing a hypothetical one.
	 *
	 * The childcare cost overlay (#135) is the `ChildcareCostPanel` section alongside the others:
	 * same whole-portfolio wiring as `OneOffCostsPanel`, and like it takes an editable list rather
	 * than fixed dials — `$lib/childcare-cost.js` supports any number of steps (a "stepped" bill),
	 * each running for its own date range rather than landing on a single date.
	 *
	 ...
	 * exists on Settings now, but this tab still doesn't read `profile`/`dob_year` from the store
	 * for any of its birth-year-driven features (milestones, retirement marker, age filter) either —
	 * a pre-existing gap #146/#172 left for this tab specifically, not one this issue was asked to
	 * close — so the partner fields follow the exact typed-in placeholder pattern #18 already set for
	 * the primary profile's own fields, one section down, rather than being the one field here that
	 * suddenly reads from the store while its neighbours don't. An "Add a partner" checkbox keeps the
	 * fields out of the way for the (still typical) solo user, and unchecking it drops the partner
	 * marker outright rather than leaving a stale one on screen.
	 */
	import {
				...
				<div class="space-y-2">
					...
					{#if includePartner}
						<div class="rounded-md border border-border bg-muted/40 p-3">
							<p class="text-xs text-muted-foreground mb-3">
								No partner form on this tab yet (#170 lives on Settings, not here), so these are
								typed in here for this forecast only — nothing is saved.
							</p>
							<div class="flex flex-wrap items-end gap-4">
								<div class="flex flex-col gap-1">
