<script>
	 ...
	 *
	 * A sixth section (issue #150) is the CSV counterpart to JSON Import above, wired to #130's
	 * `$lib/csv-import.js`: a Holdings or Debts CSV this app exported can be brought back in. Unlike
	 * JSON import it's a **merge**, not a replace — `parseCsvImport` folds the file's rows into a copy
	 * of the current document by month and returns the merged result plus counts for the confirm
	 * step, so nothing here re-derives that logic, only renders it and decides when to apply it.
	 * Net Worth History stays read-only (`parseCsvImport` rejects it with its own specific message).
	 */
	...
	import { parseCsvImport } from '$lib/csv-import.js';
	...
	/** @type {'idle' | 'confirming'} */
	let csvImportPhase = $state('idle');
	let csvImportError = $state('');
	/** @type {import('$lib/types.js').ValidationError[]} */
	let csvImportErrors = $state([]);
	let csvImportNotice = $state('');
	let csvImportBusy = $state(false);
	/** @type {import('$lib/csv-import.js').CsvImportSuccess | null} */
	let pendingCsvImport = $state(null);

	/** @param {Event} event */
	async function onCsvFileChosen(event) {
		csvImportError = '';
		csvImportErrors = [];
		csvImportNotice = '';
		...
		const text = await file.text();
		const result = parseCsvImport(text, get(appData));
		if (!result.ok) {
			csvImportError = result.message;
			csvImportErrors = result.errors;
			return;
		}
		pendingCsvImport = result;
		csvImportPhase = 'confirming';
	}

	async function confirmCsvImport() {
		if (!pendingCsvImport) return;
		csvImportBusy = true;
		const { summary } = pendingCsvImport;
		appData.set(pendingCsvImport.data);
		await flushAppDataSync();
		const state = get(syncState);
		csvImportNotice = state.error
			? `Imported, but saving it failed: ${state.error}`
			: `Imported. ${pluralise(summary.months, 'month')} updated (${summary.newMonths} new, ${summary.updatedMonths} already had data) — the rest of your data is unchanged.`;
		pendingCsvImport = null;
		csvImportPhase = 'idle';
		csvImportBusy = false;
	}
...
<div class="flex flex-col gap-6">
	...
	<Card className="p-4">
		...
		<p class="text-sm text-muted-foreground mb-3">
			...
			20 data-portability request or a quick import into another spreadsheet tool. Net Worth History is
			read-only, like the Excel export above — it's a derived total, not individual records. Holdings
			and Debts can be brought back in via "Import data from CSV" below.
		</p>
	...
	<Card className="p-4">
		<h2 class="text-lg font-semibold mb-1">Import data from CSV</h2>
		<p class="text-sm text-muted-foreground mb-3">
			Brings a Holdings or Debts CSV this app exported back in — which dataset it is is detected
			from the file's header row, not asked. Unlike JSON import above, this is a merge, not a
			replace: only the months the file mentions are updated, and everything else currently stored
			is left exactly as it was. The file is checked in full before anything changes. Net Worth
			History can't be re-imported this way — it's a derived total, not individual records.
		</p>
		...
		{:else if pendingCsvImport}
			<div class="border border-border rounded-md p-3">
				<p class="text-sm font-medium mb-1">
					This file is {CSV_DATASET_LABELS[pendingCsvImport.dataset]} data. Importing will merge:
				</p>
				<p class="text-sm text-muted-foreground mb-3">
					{pluralise(
						pendingCsvImport.summary.records,
						pendingCsvImport.dataset === 'holdings' ? 'holding' : 'debt'
					)} across {pluralise(pendingCsvImport.summary.months, 'month')} ({pendingCsvImport.summary
						.newMonths} new, {pendingCsvImport.summary.updatedMonths} already had data). The rest of your
					data is unchanged.
				</p>
				<div class="flex flex-wrap items-center gap-2">
					<Button type="button" size="sm" disabled={csvImportBusy} onclick={confirmCsvImport}>
						{csvImportBusy ? 'Importing…' : 'Merge this file into my data'}
					</Button>
					<Button
						type="button"
						variant="outline"
		...
		{#if csvImportNotice}<p class="text-sm text-green-700 mt-3" role="status">
				{csvImportNotice}
			</p>{/if}
		{#if csvImportError}
			<p class="text-sm text-red-600 mt-3" role="alert">{csvImportError}</p>
			{#if csvImportErrors.length > 0}
				<ul class="text-xs text-red-600 list-disc pl-5 mt-1">
					{#each csvImportErrors as err (err.path)}
						<li><code>{err.path}</code>: {err.message}</li>
					{/each}
				</ul>
			{/if}
