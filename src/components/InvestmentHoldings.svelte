<script>
	 ...
	 *
	 * A new month starts pre-filled from the most recent *earlier* entry (issue #259), via
	 * `$lib/model.js`'s `createNextMonthlyEntry` — most months barely change from the last, so
	 * starting from a copy beats re-entering every holding from scratch. That logic lives in
	 * `$lib/model.js` rather than here so it can be unit tested without a DOM (see
	 * `model.test.js`'s `createNextMonthlyEntry` suite); `addMonth` below just calls it.
	 *
	 ...
	 * again.
	 *
	 * `InvestmentGuidance` (issue #255) renders directly below the form as its own collapsed card —
	 * plain-English ISA and non-ISA Capital Gains Tax reference content, not another form. It lives in
	 * its own component rather than inline here because the wrapper picker's six ISA options are the
	 * thing that content explains, so a returning user who already knows it can leave it collapsed
	 * without it ever touching this component's own state.
	 *
	 * **"Update prices" (issue #300)** wires #298's `refreshInvestmentPrices` and #295's
	 * `PriceRefreshResults` together — this component owns neither fetching nor rendering the
	 * per-holding feedback, only the click-to-fetch-to-write-back plumbing between them. It acts on
	 * the *selected* month's holdings, applying results through `$lib/model.js`'s
	 * `applyPriceRefreshResults` (extracted for the same DOM-free-testability reason as
	 * `createNextMonthlyEntry`) via the same wholesale `monthlyEntries` reassignment `saveEdit` uses.
	 * The button is hidden outright when `isPriceFeedAvailable()` is false, matching #266's documented
	 * fallback of the ticker field behaving like a plain manual field with no key configured.
	 *
	 * Deliberately **not** written to `activityLog`. The log (issue #14) is a record of deliberate
	 * per-holding add/edit/remove actions with a revert affordance; one click here can touch every
	 * holding in the month, and logging each as though it were its own edit would both drown that log
	 * and offer a "revert" on an entry that was never really a discrete user decision — there is
	 * nothing sensible to revert *to* other than running another refresh. The refreshed values remain
	 * visible (and further hand-editable, which *does* log) in the holdings list itself, and #295's
	 * results panel is the record of what a given refresh did.
	 */
	import {
		INVESTMENT_TYPES,
	...
	import { isPriceFeedAvailable, refreshInvestmentPrices } from '$lib/price-feed.js';
	...
	/** Static for the component's lifetime — the API key is a build-time env var, not something that
	 * changes while the app is running. */
	const priceFeedAvailable = isPriceFeedAvailable();
	let refreshingPrices = $state(false);
	let priceRefreshResults = $state(
		/** @type {import('$lib/price-feed.js').PriceRefreshResult[]} */ ([])
	);

	const priceRefreshHoldingNames = $derived(
		Object.fromEntries((selectedEntry?.investments ?? []).map((inv) => [inv.id, inv.name]))
	);

	async function refreshPrices() {
		if (!selectedEntry || refreshingPrices) return;
		const entryId = selectedEntry.id;
		refreshingPrices = true;
		try {
			const results = await refreshInvestmentPrices(selectedEntry.investments);
			monthlyEntries = monthlyEntries.map((entry) =>
				entry.id === entryId
					? { ...entry, investments: applyPriceRefreshResults(entry.investments, results) }
					: entry
			);
			priceRefreshResults = results;
		} finally {
	...
	function addMonth() {
		...
		const entry = createNextMonthlyEntry(monthlyEntries, { month, year });
	...
	function startEdit(investment) {
		...
		ticker = investment.ticker === null ? '' : investment.ticker;
	}
	...
	function saveEdit() {
		...
		// `last_price` is not on the form, so it survives an edit by default — but it is only
		// meaningful paired with the `value`/`ticker` it was fetched against, so retyping either of
		// those drops it rather than leaving a stale multiplier behind (see `carryLastPrice`).
		const after = { ...before, ...fields, last_price: carryLastPrice(before, fields) };
		const entryId = selectedEntry.id;
		monthlyEntries = monthlyEntries.map((entry) =>
			entry.id === entryId
				? {
...
<div class="flex flex-col gap-6">
		...
		{:else}
			<div class="flex flex-wrap items-center justify-between gap-3 mb-3">
				...
				{#if priceFeedAvailable}
					...
					>
						{refreshingPrices ? 'Updating prices…' : 'Update prices'}
					</Button>
				{/if}
			</div>
			...
			>
				...
				<div class="flex flex-col gap-1">
					<label class="text-sm font-medium" for="holding-ticker"
						>Ticker symbol (optional, e.g. VWRL.L)</label
					>
					<input
						id="holding-ticker"
						type="text"
	...
	{#if priceFeedAvailable}
		<PriceRefreshResults results={priceRefreshResults} holdingNames={priceRefreshHoldingNames} />
	{/if}
