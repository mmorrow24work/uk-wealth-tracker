 ...
 * - `--chart-1`/`--chart-2` at 3:1 against both surfaces they're drawn on (the page background and
 *   the card they usually sit inside), the bar the existing `--chart-1` comment in `app.css` sets;
 * - `--chart-1` vs `--chart-2` at ΔE ≥ 8 from each other under normal vision *and* under protanope,
 *   deuteranope and tritanope simulation, so a palette whose two series happen to collapse into one
 *   colour for a red-green-blind reader fails here rather than in the wild.
 *
...
describe('palette contrast', () => {
	...
	it.each(COMBINATIONS)(
		'%s/%s keeps both chart series at 3:1 on both surfaces',
		(name, variant) => {
			const tokens = resolveTokens(name, variant);
	...
	it.each(COMBINATIONS)('%s/%s keeps its two chart series apart under CVD', (name, variant) => {
		const tokens = resolveTokens(name, variant);
		expect(worstCaseSeparation(tokens['--chart-1'], tokens['--chart-2'])).toBeGreaterThanOrEqual(8);
	});
});
