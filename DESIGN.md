...
A UK-centric personal wealth tracker with sections that work together to capture net worth, tax and retirement planning on a self-hosted, GitHub Pages-deployed equivalent with full data ownership via GitHub Gist persistence.
...
- JSON export/import works in both modes, as the common interchange format between them; XLSX export and the Net Worth History CSV are secondary, read-only data paths (WealthR data portability) — but the Holdings/Debts CSV export can be brought back in via CSV import (#130/#150), as a per-month merge rather than JSON import's full replace
...
**Status (#130, #150):** this app's own CSV export format (`csv-export.js`, #129) is round-trip importable — `src/lib/csv-import.js` (#130) parses, validates and merges a Holdings or Debts CSV this app itself produced back into `AppData`, and the Settings tab's "Import data from CSV" card (#150) wires a file picker, confirm step and error reporting up to it, alongside JSON import. Net Worth History stays read-only (it is a derived total with no way back to the individual records that produced it). WealthR's own export format is still out of scope: no real WealthR CSV sample has been available to build a column mapping against, and guessing at an unverified layout risks silently mis-importing someone's real financial data. Unblocking it needs an actual WealthR export file to work from.
